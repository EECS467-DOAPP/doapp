...
TrajectoryFollower::TrajectoryFollower()
    : port_("/dev/ttyUSB0"), protocol_(2.0), baud_rate_(2000000),
      prev_waypt_(0), next_waypt_(0) {
  ...
  point.positions = {117.14 * DEG2RAD,
                     25.78 * DEG2RAD,
                     52.26 * DEG2RAD,
                     70.79 * DEG2RAD,
                     0,
...
void TrajectoryFollower::set_port(const std::string &port) { port_ = port; }

void TrajectoryFollower::set_protocol(double protocol) { protocol_ = protocol; }
...
void TrajectoryFollower::initialize_dynamixel() {
  if (!wb_.init(port_.c_str(), baud_rate_)) {
    ROS_ERROR("Could not initialize dynamixel at port %s with baud rate %d",
              port_.c_str(), baud_rate_);
    ros::shutdown();
  }
  ROS_INFO("Succeeded to initialize port %s with baud rate %d", port_.c_str(),
           baud_rate_);
...
void TrajectoryFollower::trajectory_callback(
    const trajectory_msgs::JointTrajectoryConstPtr &trajectory_msg) {
  std::lock_guard<std::mutex> lock(mtx_);
  current_trajectory_ = *trajectory_msg;
  std::cout << "NEW TRAJECTORY WITH " << current_trajectory_.points.size()
            << " POINTS" << std::endl;
  prev_waypt_ = 0;
...
void TrajectoryFollower::gripper_callback(
    const std_msgs::Float64ConstPtr &trajectory_msg) {
  std::lock_guard<std::mutex> lock(mtx_);
...
sensor_msgs::JointState TrajectoryFollower::follow_trajectory() {
  ...
  if (prev_waypt_ == next_waypt_) {
    ...
    for (size_t i = 0; i < joints_.num_joints; i++) {
      state.position[i] = (1.0 - segment_progress) * prev_point.positions[i] +
                          segment_progress * next_point.positions[i];
    }
  }
  ...
  for (size_t i = 0; i < joints_.num_joints; i++) {
    goal_data.push_back(
        wb_.convertRadian2Value(joints_.joint_ids[i], goal_angle[i]));
    goal_data.push_back(
        wb_.convertVelocity2Value(joints_.joint_ids[i], goal_velocity[i]));
  }

  int32_t *write_data = reinterpret_cast<int32_t *>(goal_data.data());
  if (!wb_.syncWrite(0, joints_.joint_ids.data(), joints_.num_joints,
                     write_data, 1)) {
