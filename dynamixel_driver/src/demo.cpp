 ...
 *          from other node. The message type is ArmCommand
 ...
 */
void init(dynamixel_driver::ArmCommand &msg) {
...
int main(int argc, char **argv) {
    ...
    ros::NodeHandle n;
    ros::ServiceClient client = n.serviceClient<dynamixel_driver::ArmCommand>("arm_command");
    ...
    dynamixel_driver::ArmCommand msg;
    demo::init(msg);
    int id = 3;
    ...
    while (ros::ok()) {
        ...
        if (client.call(msg)) {
            ROS_INFO("Publishing %d to arm [%d]\n", msg.request.commands[id], id);
