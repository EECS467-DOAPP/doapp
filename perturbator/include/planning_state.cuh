...
public:
  ...
  void updateArmState(const sensor_msgs::JointState::ConstPtr &msg) {
    ...
    current_state =
        std::vector<float>(msg->position.begin(), msg->position.end());
...
private:
  ...
  const float viable_score_max =
      100; // literally have no clue what this should be, other than the cost of
           // one obstacle collision
  bool planning_trajectory = false, _have_goal;
  unsigned int num_initial_trajectories, num_noise_vectors, num_waypoints,
      waypoint_dim;
