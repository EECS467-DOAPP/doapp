...
PlanningState::PlanningState(unsigned int num_initial_trajectories_,
                             unsigned int num_noise_vectors_,
                             unsigned int num_waypoints_,
                             unsigned int waypoint_dim_,
                             const doapp::cpu_distance_grid::Dimensions &dims)
    : num_initial_trajectories(num_initial_trajectories_),
      num_noise_vectors(num_noise_vectors_), num_waypoints(num_waypoints_),
      waypoint_dim(waypoint_dim_), _have_goal(false),
      trajectories(num_initial_trajectories_ * num_waypoints_ * waypoint_dim_),
      noise_vectors(num_initial_trajectories_ * num_noise_vectors_ *
                    waypoint_dim_),
      noisy_trajectories(num_initial_trajectories_ * num_noise_vectors_ *
                         num_waypoints_ * waypoint_dim_),
      rngs(num_initial_trajectories_ * waypoint_dim_ *
           std::max(num_noise_vectors_, num_waypoints_)),
      velocities(num_initial_trajectories_ * num_noise_vectors_ *
                 num_waypoints_ * waypoint_dim_),
      accelerations(num_initial_trajectories_ * num_noise_vectors_ *
                    num_waypoints_ * waypoint_dim_),
      smoothness(num_initial_trajectories_ * num_noise_vectors_),
      scores(num_initial_trajectories_ * (num_noise_vectors_ + 1)),
      update_vectors(num_initial_trajectories_ * waypoint_dim_), flag(1),
      found_better(num_initial_trajectories_),
      grid_ptr(doapp::make_unique<doapp::CPUDistanceGrid>(dims)) {
  auto num_rngs = num_initial_trajectories * waypoint_dim *
                  std::max(num_noise_vectors, num_waypoints);
  init_cudarand<<<ceil(double(num_rngs) / double(512)), 512>>>(rngs.data(),
                                                               num_rngs);
...
void PlanningState::plan() {
  ...
  if (cudaMemcpyToSymbol(initial_waypoint, current_state.data(),
                         current_state.size() * sizeof(float), 0,
                         cudaMemcpyHostToDevice) != cudaSuccess) {
    ROS_ASSERT(false);
  }
  if (cudaMemcpyToSymbol(final_waypoint, goal_state.data(),
                         goal_state.size() * sizeof(float), 0,
                         cudaMemcpyHostToDevice) != cudaSuccess) {
    ROS_ASSERT(false);
  }
  if (cudaMemcpyToSymbol(initial_trajectory, lerp.data(),
                         lerp.size() * sizeof(float), 0,
                         cudaMemcpyHostToDevice) != cudaSuccess) {
    ROS_ASSERT(false);
  }
  flag[0] = false;
  optimize_trajectories<<<gridDim, blockDim>>>(
      trajectories.data(), noise_vectors.data(), noisy_trajectories.data(),
      rngs.data(), velocities.data(), accelerations.data(), smoothness.data(),
      scores.data(), update_vectors.data(), found_better.data(), flag.data(),
      waypoint_dim * std::max(num_waypoints, num_noise_vectors), num_waypoints,
      waypoint_dim, num_noise_vectors, deltaT, *grid_ptr);
...
void PlanningState::publish_trajectory(ros::Publisher &pub) {
  ...
  for (size_t i = 0; i < num_initial_trajectories * (num_noise_vectors + 1);
       ++i) {
  ...
  bool publish_initial_trajectory =
      best_trajectory_index % (num_noise_vectors + 1) == 0;
  ...
  if (publish_initial_trajectory) {
    ...
    for (size_t i = 0; i < num_waypoints - 1; ++i) {
      for (size_t d = 0; d < waypoint_dim; ++d) {
        lerp_vels[i * waypoint_dim + d] =
            (lerp[(i + 1) * waypoint_dim + d] - lerp[i * waypoint_dim + d]) /
            deltaT;
      }
    }
  ...
  float *best_trajectory =
      publish_initial_trajectory
          ? lerp.data()
          : noisy_trajectories.data() +
                (best_trajectory_index * num_waypoints * waypoint_dim);
  float *best_velocity =
      publish_initial_trajectory
          ? lerp_vels.data()
          : velocities.data() +
  ...
  for (size_t i = 0; i < waypoint_dim; ++i) {
    point.positions.push_back(current_state[i]);
    point.velocities.push_back((best_trajectory[i] - current_state[i]) /
                               deltaT);
  }
  point.time_from_start = ros::Duration(0);
  msg.points.push_back(point);
  ...
  for (size_t i = 0; i < num_waypoints; ++i) {
    for (size_t j = 0; j < waypoint_dim; ++j) {
      ...
      point.velocities.push_back(
          best_velocity[(i + 1) * waypoint_dim +
                        j]); // due to how velocities are computed on GPU
